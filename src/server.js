...
// Embeddable widget endpoint (protected)
...
function requireAuth(req, res, next) {
  ...
  if (isConfigPreview) {
    // Allow config page previews without authentication
    return next();
  }
  ...
  if (token && verifyApiKey(token)) {
    // Valid token provided in URL
    return next();
  }
  
  // No valid token - serve authentication page
...
// Root page - Direct embed with defaults (protected)
